(function(){
  ...
  const VERSION='20260827-dock-bg-soft-edge-1';
  const W=1080,H=1350;
  ...
  const S={img:null,athleteFile:null,cutoutMask:null,cutoutBusy:false,cutoutEdge:2.5,cutoutShadow:18,scale:1,x:0,y:0,drag:false,sx:0,sy:0,ix:0,iy:0,first:'WYLDER',last:'SMITH',classLine:'CLASS OF 2027',cardNameDirty:false,overlay:null,atlas:null,dockBg:null,ready:false};
...
function drawRasterText(ctx,text,style,x,y,height,tracking,maxWidth){
  ...
  function drawDockBackground(ctx){
    if(S.dockBg){
      ctx.save();
      ctx.filter='blur(2px)';
      ctx.drawImage(S.dockBg,-10,-10,W+20,H+20);
      ctx.restore();
    }
  ...
  function refineCutoutEdges(canvas){
    ...
    for(let i=0;i<p.length;i+=4){
      ...
      let nearTransparent=a<232;
      if(!nearTransparent&&x>0&&x<w-1&&y>0&&y<h-1){
        const n=[i-4,i+4,i-w*4,i+w*4];
        nearTransparent=n.some(j=>p[j+3]<212);
      }
      if(!nearTransparent)continue;
      const foliage=(g>68&&greenLead>5&&sat>12)||(g>88&&g>r+3&&g>b+3&&sat>8);
      const yellowGreen=(g>82&&r>70&&b<92&&g>b+14&&sat>18);
      if(foliage||yellowGreen){
        const strength=Math.min(1,Math.max(0,(greenLead+sat-12)/78));
        p[i+3]=Math.round(a*(1-.86*strength));
        p[i]=Math.round(r*(1-.28*strength)+224*.28*strength);
        p[i+1]=Math.round(g*(1-.62*strength)+228*.62*strength);
        p[i+2]=Math.round(b*(1-.28*strength)+228*.28*strength);
      }else if(a<80&&sat>30){
        p[i+3]=Math.round(a*.55);
      }
    }
