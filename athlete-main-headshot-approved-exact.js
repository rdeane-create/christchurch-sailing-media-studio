(function(){
  ...
  const VERSION='20260830-clean-athlete-cutout-8';
  const W=1080,H=1350;
  ...
  function setCutoutStatus(message){const el=q('aCutoutStatus');if(el)el.textContent=message||'';}
  ...
  async function removeAthleteBackground(){
    ...
    const btn=q('aRemoveBg');const old=btn?btn.textContent:'Remove Background';
    try{
      S.cutoutBusy=true;if(btn){btn.disabled=true;btn.textContent='Removing...';}
  ...
  async function renderSavedCards(existingCards){
    ...
    window.__CSMS_SAVED_HEADSHOT_CARDS_LOADED=true;
    ...
    for(const card of shown){
      ...
      row.innerHTML=`<div style="width:58px;height:72px;border-radius:8px;border:1px solid #d8e2ed;background:linear-gradient(#eef4f6,#d8e2ed 56%,#071e30);display:flex;align-items:center;justify-content:center;font-weight:800;color:#07152f;font-size:11px">PNG</div><div><div style="font-weight:800;font-size:14px">${escapeSavedName(String(card.name||'Saved Card').replace(/\.png$/i,''))}</div><div class="hint" style="margin-top:3px">${savedCardKind(card)} • ${when}</div></div><div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end"><a class="tiny secondary" data-action="open" target="_blank" rel="noopener" style="text-decoration:none;padding:7px 10px" href="${card.url||SAVED_HEADSHOT_FOLDER_URL}">Open/Edit</a><button type="button" class="tiny primary" data-action="download">Download</button><button type="button" class="tiny secondary" data-action="delete">Delete</button></div>`;
  ...
  async function saveFinishedCard(){
    ...
    try{
      ...
      if(window.__CSMS_SAVED_HEADSHOT_CARDS_LOADED)await renderSavedCards();
...
function drawRasterText(ctx,text,style,x,y,height,tracking,maxWidth){
  ...
  function drawDockBackground(ctx){
    ...
    const top=ctx.createLinearGradient(0,0,0,350);
    top.addColorStop(0,'rgba(255,255,255,.78)');top.addColorStop(.52,'rgba(255,255,255,.22)');top.addColorStop(1,'rgba(255,255,255,0)');
  ...
  function drawPhoto(ctx){
    ...
    const foot=ctx.createLinearGradient(0,760,0,H);
  ...
  function init(){
    buildCard();ensureSavedCardsPanel();
    document.addEventListener('click',e=>{
      ...
      if((b.dataset&&b.dataset.workspace==='media')||text==='library')setTimeout(()=>{ensureSavedCardsPanel()},80);
    },true);
