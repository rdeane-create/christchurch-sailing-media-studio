(function(){
  ...
  const VERSION='20260814-contour-kern-12';
  const W=1080,H=1350;
  ...
  function loadImage(src){return new Promise((res,rej)=>{const i=new Image();i.onload=()=>res(i);i.onerror=()=>rej(new Error('Exact approved local asset unavailable: '+src));i.src=src+'?v=20260814-contour-kern-12';});}
...
function contourDelta(style,a,b,height,tracking){
  ...
  const target=style==='large'?1.5:4;
  const needs=[];
  const samples=Math.max(24,Math.round(height));
  for(let n=0;n<samples;n++){
    const t=samples===1?0:n/(samples-1);
    if(t<.10||t>.90)continue;
  ...
  needs.sort((x,y)=>x-y);
  const q=needs[Math.floor((needs.length-1)*.82)];
  const min=-height*.34,max=style==='large'?height*.04:height*.12;
