...
# SCOPE CONTROL: preserve top fade, wedge geometry, orange-to-bottom treatment,
# type, sizing, and animation exactly. Change ONLY the athlete-side edge blend.
old_feather = """  lx.save();
...
new_feather = """  // Smooth directional feather on ONLY the athlete-side edge. Rather than
...
if old_feather not in block:
    raise RuntimeError('Could not find current v17 vertical feather')
...
assert "20260815-welcome-athlete-main-drive-v18-smooth-directional-edge" in s
assert "const featherW=76;" in check
assert "const edgeFade=lx.createLinearGradient(-featherW,0,4,0);" in check
assert "edgeFade.addColorStop(.54,'rgba(244,81,30,.22)')" in check
...
p.write_text(s)
print('Installed Welcome Aboard smooth directional edge v18')
