...
const loadScript=src=>new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src+'?v=20260811-locked-v3';s.onload=()=>{s.remove();resolve();};s.onerror=()=>reject(new Error('Could not load '+src));document.head.appendChild(s);});
const loadHeaderChunk=async src=>{
  const response=await fetch(src+'?v=20260811-locked-v3',{cache:'no-store'});
...
(async()=>{try{
  ...
  for(const f of headerFiles)window.__CSMS_HERO_HEADER_B64+=await loadHeaderChunk(f);
