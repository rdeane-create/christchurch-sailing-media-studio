(function(){
'use strict';
const VERSION='20260815-welcome-original-restored-v1';
...
function injectStyles(){
  ...
  #workspace-welcome canvas{width:min(100%,540px);height:auto;box-shadow:0 8px 30px rgba(0,0,0,.15);background:#06142c}
...
function ensureWorkspace(){
  ...
  <div id="welcomeStatus" class="status">Choose a saved Hero Card.</div></section>
  ...
  <div class="hint">Hero Card master + locked WELCOME ABOARD overlay • 1080 × 1350 • 4:5</div></section></div></div>`;
...
function drawWelcomeOverlay(ctx,progress=1){const p=Math.max(0,Math.min(1,progress)),eased=1-Math.pow(1-p,3),finalX=585,finalY=1062,startX=1180,startY=1420,x=startX+(finalX-startX)*eased,y=startY+(finalY-startY)*eased,angle=(-11*Math.PI/180)+((1-p)*5*Math.PI/180);ctx.save();ctx.translate(x,y);ctx.rotate(angle);const w=560,h=230;ctx.save();ctx.translate(14,18);roundedRectPath(ctx,0,0,w,h,5);ctx.fillStyle='rgba(2,18,40,.86)';ctx.fill();ctx.restore();roundedRectPath(ctx,0,0,w,h,5);ctx.fillStyle='#f4511e';ctx.fill();ctx.lineWidth=3;ctx.strokeStyle='rgba(255,248,236,.86)';ctx.stroke();ctx.save();ctx.translate(12,12);roundedRectPath(ctx,0,0,w-24,h-24,3);ctx.lineWidth=1.5;ctx.strokeStyle='rgba(255,248,236,.50)';ctx.stroke();ctx.restore();ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#fff8ec';ctx.strokeStyle='rgba(6,26,49,.22)';ctx.lineWidth=2;ctx.shadowColor='rgba(2,18,40,.28)';ctx.shadowBlur=3;ctx.shadowOffsetY=2;const family='"Avenir Next Condensed","Helvetica Neue Condensed","Arial Narrow",Impact,sans-serif';ctx.font=`italic 900 72px ${family}`;ctx.strokeText('WELCOME',w/2,77);ctx.fillText('WELCOME',w/2,77);ctx.font=`italic 900 78px ${family}`;ctx.strokeText('ABOARD',w/2,155);ctx.fillText('ABOARD',w/2,155);ctx.restore();}
function drawWelcomeCard(){const e=welcomeEls();if(!e.canvas)return;const ctx=e.canvas.getContext('2d');ctx.clearRect(0,0,e.canvas.width,e.canvas.height);if(welcomeHeroImage){const iw=welcomeHeroImage.naturalWidth||welcomeHeroImage.width||1080,ih=welcomeHeroImage.naturalHeight||welcomeHeroImage.height||1350,scale=Math.max(1080/iw,1350/ih),dw=iw*scale,dh=ih*scale;ctx.drawImage(welcomeHeroImage,(1080-dw)/2,(1350-dh)/2,dw,dh);drawWelcomeOverlay(ctx,welcomeDropProgress);}else{ctx.fillStyle='#06142c';ctx.fillRect(0,0,1080,1350);ctx.fillStyle='rgba(255,255,255,.86)';ctx.textAlign='center';ctx.font='600 34px Arial,sans-serif';ctx.fillText('Select an available Hero Card',540,675);}}
