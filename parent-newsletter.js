...
(function () {
  ...
  try {
    ...
    if (saved && Array.isArray(saved.sections) && saved.sections.length === titles.length) {
      ...
      state.sections.forEach((section,i)=>{if(Array.isArray(saved.sections[i]?.extraLinks))section.extraLinks=extraLinks(saved.sections[i]);});
    }
  ...
  function extraLinks(section) {return Array.isArray(section.extraLinks)?section.extraLinks.filter(l=>l&&typeof l.url==='string'&&typeof l.label==='string'):[];}
  function sectionLinks(section) {return [{url:section.link,label:section.linkLabel},...extraLinks(section)].filter(l=>safeUrl(l.url));}
  ...
  function linkControls(host,section,index) {
    const list=document.createElement('div'),add=document.createElement('button');
    add.type='button';add.className='secondary tiny';add.textContent='Add another link';
    function row(link) {
      const box=document.createElement('div');box.dataset.section='links-'+index+'-'+crypto.randomUUID();
      box.style.cssText='border:1px solid #d8e2ed;padding:12px;margin:12px 0;border-radius:10px';
      field(box,link,'url','Additional link URL',false,'url');field(box,link,'label','Additional link text');
      const remove=document.createElement('button');remove.type='button';remove.className='secondary tiny';remove.textContent='Remove link';
      remove.onclick=()=>{section.extraLinks=section.extraLinks.filter(l=>l!==link);box.remove();update();};box.append(remove);list.append(box);
    }
    extraLinks(section).forEach(row);
