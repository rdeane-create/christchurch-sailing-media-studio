...
(function () {
  ...
  function mediaSlots() {return ['hero',...state.sections.flatMap((s,i)=>s.visible?[`photo-${i}`,`video-${i}`]:[])];}
  ...
  try {
    ...
    if(saved?.mediaRefs && typeof saved.mediaRefs==='object' && !Array.isArray(saved.mediaRefs)) mediaRefs=Object.fromEntries(Object.entries(saved.mediaRefs).filter(([k,v])=>/^(hero|(?:photo|video)-[0-7])$/.test(k)&&typeof v==='string'));
  ...
  function uploadControls(host,slot,label,kind) {
    ...
    input.type='file';input.id='pn-upload-'+slot;input.accept=kind==='photo'?'image/jpeg,image/png,image/webp,image/gif':'video/mp4,video/webm,video/quicktime';l.htmlFor=input.id;l.textContent=label;
    ...
    input.onchange=async()=>{
      ...
      try {
        ...
        if(!allowed.includes(file.type))throw Error('Choose a supported '+kind+' file.');
        ...
        const record={id:'pn-'+crypto.randomUUID(),name:file.name,type:file.type,size:file.size,data};
